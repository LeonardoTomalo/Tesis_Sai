...
public class TipoTransporteController {
    ...
    @FXML private JFXListView<TipoTransporte> lst_lista;
	...
	String urlBase = PropertyManager.getBaseUrl();
 	String uriDisponibilidad = urlBase + "/tipoTransporte";
    
 	// DE LA CLASE IDIOMAS
 	TipoTransporte dispCelular = new TipoTransporte();
 	List<TipoTransporte> listadispCelular = new ArrayList<TipoTransporte>();
 	private static ResponseEntity<List<TipoTransporte>> listRespdispCelular;
	ObservableList<TipoTransporte> obsListdispCelular = FXCollections.observableArrayList();
    ...
    void addNuevo(ActionEvent event) {
		initialize();
		dispCelular = new TipoTransporte();
    }
    ...
    void guardar(ActionEvent event) {
    	try {
			...
			if (result.get() == ButtonType.OK) {
				...
				rest.postForObject(uriDisponibilidad + "/saveOrUpdate", dispCelular, TipoTransporte.class);
	...
	private void loaddispCelular() {
		...
		if (!listadispCelular.isEmpty()) {
	    	...
	    	lst_lista.setCellFactory(param -> new ListCell<TipoTransporte>() {
		...
		lst_lista.getSelectionModel().selectedItemProperty()
				.addListener((ObservableValue<? extends TipoTransporte> ov, TipoTransporte old_val, TipoTransporte new_val) -> {
