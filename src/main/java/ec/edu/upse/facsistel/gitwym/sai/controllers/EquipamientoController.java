...
public class EquipamientoController {
    ...
    @FXML private JFXListView<Equipamiento> lst_lista;
	...
	String urlBase = PropertyManager.getBaseUrl();
 	String uriDisponibilidad = urlBase + "/equipamiento";
    
 	// DE LA CLASE IDIOMAS
 	Equipamiento dispCelular = new Equipamiento();
 	List<Equipamiento> listadispCelular = new ArrayList<Equipamiento>();
 	private static ResponseEntity<List<Equipamiento>> listRespdispCelular;
	ObservableList<Equipamiento> obsListdispCelular = FXCollections.observableArrayList();
    ...
    void guardar(ActionEvent event) {
    	try {
			...
			if (result.get() == ButtonType.OK) {
				...
				rest.postForObject(uriDisponibilidad + "/saveOrUpdate", dispCelular, Equipamiento.class);
	...
	private void loaddispCelular() {
		...
		if (!listadispCelular.isEmpty()) {
	    	...
	    	lst_lista.setCellFactory(param -> new ListCell<Equipamiento>() {
	    		protected void updateItem(Equipamiento item, boolean empty) {
		...
		lst_lista.getSelectionModel().selectedItemProperty()
				.addListener((ObservableValue<? extends Equipamiento> ov, Equipamiento old_val, Equipamiento new_val) -> {
